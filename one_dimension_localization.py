...
def main(forecast, real, iterations=20, threshold=0.7, topK=10):
    ...
    choice = {'app_name': list((forecast['impression'].groupby(forecast.app_name).sum() - real[
        'impression'].groupby(real.app_name).sum()).reset_index().sort_values('impression', ascending=False)[
                               0:topK].app_name),
              'country': list((forecast['impression'].groupby(forecast.country).sum() - real['impression'].groupby(
                  real.country).sum()).reset_index().sort_values('impression', ascending=False)[0:topK].country),
              'id_type': list((forecast['impression'].groupby(forecast.id_type).sum() - real['impression'].groupby(
                  real.id_type).sum()).reset_index().sort_values('impression', ascending=False)[0:topK].id_type),
              'platform':list((forecast['impression'].groupby(forecast.platform).sum() - real['impression'].groupby(
                  real.platform).sum()).reset_index().sort_values('impression', ascending=False)[0:topK].platform),
              'tu': list((forecast['impression'].groupby(forecast.tu).sum() - real['impression'].groupby(
                  real.tu).sum()).reset_index().sort_values('impression', ascending=False)[0:topK].tu)}
    print(choice)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--threshold', default=0.65, type=float, help='the threshold of stopping searching MCTS. \
                                            Set 1 means do not stop searching until reaching max iterations')
    parser.add_argument('-k', '--topK', default=10, type=int, help='the topK choice for one attribution')
