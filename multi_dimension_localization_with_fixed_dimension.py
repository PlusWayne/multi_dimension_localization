...
# SETTINGS
ATTR = ['app_name', 'country', 'id_type', 'platform', 'tu']
TARGET = 'impression'
FIXED = {}
for att in ATTR:
    FIXED[att] = []  # 需要输入一个类别的空dict
# if you want to fix some values in some dimensions, just put this values in the right place
# for example, we want to make 'platform' dimension only have two values 'admob' and 'sniper'
# we can use FIXED['platform'] = ['admob', 'sniper']
# for multiple dimensions, just do it multiple times with a loop.
...
class MCTS(object):
    ...
    def get_scores(self, root_cause, forecast, real):
        ...
        total_h = (forecast.set_index(ATTR) - real.set_index(ATTR)).dropna(axis=0)[self.target].apply(lambda x: x ** 2).sum()
        if total_h == 0:
        ...
        temp = (f.set_index(ATTR) - real.set_index(ATTR)).dropna(axis=0)[self.target].apply(lambda x: x ** 2).sum()
...
def preprocessing_data(forecast, real, target, attributions):
    ...
    forecast = forecast[TARGET].groupby \
        (by=[forecast[str(x)] for x in ATTR]).sum().reset_index()
    forecast = forecast.replace(['none', ''], np.nan).dropna().reset_index(drop=True)  # drop nan
    if 'tu' in ATTR:
        forecast.tu = forecast.tu.astype(int)
    real = real[TARGET].groupby \
        (by=[real[str(x)] for x in ATTR]).sum().reset_index()
    ...
    temp = temp[temp[ATTR].duplicated()]
    real = temp[TARGET].groupby \
        (by=[temp[str(x)] for x in ATTR]).min().reset_index()
    temp = pd.concat([forecast, real])
    uniq = temp.drop_duplicates(subset=ATTR, keep=False)
    uniq[TARGET] = 0
    real = pd.concat([real, uniq])
    return forecast, real

# def preprocessing_data_fixed_dimension(FIXED, forecast, real):
#     for attr, val in FIXED.items():
#         if len(val) >0:
#             mask_forecast = forecast[attr].isin(val)
#             mask_real = real[attr].isin(val)
#             forecast = forecast[mask_forecast]
#             real = real[mask_real]
#     return forecast, real
...
def groupToOthers(forecast, real, topK):
    for attr in ATTR:
        rank = forecast[TARGET].groupby(forecast[attr]).sum().sort_values(ascending=False).reset_index()
        mask_forecast = ~forecast[attr].isin(list(rank[0:topK][attr]))
        mask_real = ~real[attr].isin(list(rank[0:topK][attr]))
        if attr != 'tu':
            forecast.loc[mask_forecast, attr] = 'other_' + attr
            real.loc[mask_real, attr] = 'other_' + attr
        else:
            forecast.loc[mask_forecast, attr] = 0
            real.loc[mask_real, attr] = 0
    forecast = forecast.groupby(by=ATTR).sum().reset_index()
    real = real.groupby(by=ATTR).sum().reset_index()
    print(len(forecast))
    return forecast, real
...
def main(forecast, real, iterations=20, threshold=0.7, topK=10):
    ...
    choice = {}
    for attr in ATTR:
        choice[attr] = list(forecast[attr].unique())
    print(choice)
    root_cause_list = []
...
if __name__ == '__main__':
    ...
    forecast, real = preprocessing_data(forecast, real, target=TARGET,
                                        attributions=ATTR)
    # forecast, real = preprocessing_data_fixed_dimension(FIXED, forecast, real)
